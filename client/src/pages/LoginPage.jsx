...
const LoginPage = () => {
  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      {/* right */}
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-8 flex flex-col gap-6 rounded-lg shadow-lg w-full max-w-lg sm:max-w-lg'>
        ...
        <div className='flex flex-col gap-2'>
          {currentState === "Sign Up" ? (
            ...
            <p  className='text-sm text-gray-600'> Create an account <span className='font-medium text-violet-500 cursor-pointer' onClick={()=>{setCurrentState("Sign Up"); setIsDataSubmitted(false)}}>Click here</span></p>
          )}
        </div>
      </form>
    </div>
